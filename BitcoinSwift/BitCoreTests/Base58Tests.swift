//
//  Base58Tests.swift
//  BitCoreTests
//
//  Created by SPARK-Daniel on 2021/12/22.
//

import XCTest

@testable import BitCore

class Base58Tests: XCTestCase {

    override func setUpWithError() throws {
        // Put setup code here. This method is called before the invocation of each test method in the class.
    }

    override func tearDownWithError() throws {
        // Put teardown code here. This method is called after the invocation of each test method in the class.
    }

    func testBase58Encode() throws {
        for vector in testVectors {
            HexEncodesToBase58(hex: vector[0], base58: vector[1])
        }
    }
    
    func testBase58() {
        HexEncodesToBase58(hex: "00010966776006953d5567439e5e39f86a0d273beed61967f6", base58: "16UwLL9Risc3QfPqBUvKofHmBQ7wMtjvM")
    }
    
    func testBase58Decode() throws {
        for vector in testVectors {
            Base58DecodesToHex(hex: vector[0], base58: vector[1])
        }
    }
    
    
    func HexEncodesToBase58(hex: String, base58: String) {
        //Encode
        let data = Data(hex: hex)
        XCTAssertEqual(Base58.encode(data), base58)
    }
    
    func Base58DecodesToHex(hex: String, base58: String) {
        XCTAssertEqual(Base58.decode(base58)!.hex, hex)
    }
    
    /// Test vectors from: https://github.com/bitcoin/bitcoin/blob/master/src/test/data/base58_encode_decode.json
    private let testVectors = [
        ["", ""],
        ["61", "2g"],
        ["626262", "a3gV"],
        ["636363", "aPEr"],
        ["73696d706c792061206c6f6e6720737472696e67", "2cFupjhnEsSn59qHXstmK2ffpLv2"],
        ["00eb15231dfceb60925886b67d065299925915aeb172c06647", "1NS17iag9jJgTHD1VXjvLCEnZuQ3rJDE9L"],
        ["516b6fcd0f", "ABnLTmg"],
        ["bf4f89001e670274dd", "3SEo3LWLoPntC"],
        ["572e4794", "3EFU7m"],
        ["ecac89cad93923c02321", "EJDM8drfXA6uyA"],
        ["10c8511e", "Rt5zm"],
        ["00000000000000000000", "1111111111"],
        ["000111d38e5fc9071ffcd20b4a763cc9ae4f252bb4e48fd66a835e252ada93ff480d6dd43dc62a641155a5", "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"],
        ["", ""]
    ]


}
