...
class Murmur3Tests: XCTestCase {
    ...
    func testExample() throws {
        ...
        for line in testdata.split(separator: "\n") {
            ...
            XCTAssertEqual(Murmur3.hash(data.data(using: .ascii)!, seed: seed), expect)
        }
    }
    
    func testMurmurHash() {
        let digest = Murmur3.hash("Hello World! Hello World!".data(using: .ascii)!)
        XCTAssertEqual(digest, 0x0be480fc)
        
        let digest1 = Murmur3.hash("Hello World! Hello World!".data(using: .ascii)!, seed: 0x7fffffff)
        XCTAssertEqual(digest1, 0x47fcc800)
    }
}
